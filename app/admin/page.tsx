        ...
        <div className="min-h-screen bg-[#f8f9fa] p-4 md:p-8 font-sans text-gray-800">
            <div className="max-w-5xl mx-auto">
                <header className="mb-10 flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                    <div>
                        <h1 className="text-3xl font-extrabold text-emerald-900 tracking-tight">Administrace Vín</h1>
                        <p className="text-gray-500 mt-1">Správa katalogu maďarských vín</p>
                    </div>
                </header>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1">
                        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-lg border border-gray-50 sticky top-8">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-1 pt-2">
                                    <label className="text-xs font-bold uppercase text-gray-400">Média a popis</label>
                            ...
                            <div className="flex flex-col gap-2 mt-6">
                                <button disabled={loading} type="submit" className={`w-full py-4 rounded-xl font-bold text-white shadow-lg transition-all ${loading ? 'bg-gray-400' : 'bg-linear-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 active:scale-[0.98]'}`}>
                                    {loading ? 'Pracuji...' : editingId ? 'Uložit změny' : 'Přidat produkt'}
                                </button>
                                {editingId && (
                                    <button type="button" onClick={resetForm} className="w-full py-2 text-sm font-semibold text-gray-500 hover:text-gray-700 transition-colors">
                                        Zrušit úpravy
                                    </button>
                                )}
                            </div>
                    ...
                    <div className="lg:col-span-2">
                        <div className="bg-white rounded-2xl shadow-lg border border-gray-50 overflow-hidden">
                            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                                <h2 className="text-xl font-bold text-gray-800">Aktuální Nabídka</h2>
                                <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded tracking-widest">{products.length} POLOŽEK</span>
                            </div>
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    <thead className="bg-gray-50/50">
                                        <tr>
                                            <th className="py-4 px-6 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Produkt</th>
                                            <th className="py-4 px-6 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Parametry</th>
                                            <th className="py-4 px-6 text-right text-xs font-bold text-gray-400 uppercase tracking-wider">Cena</th>
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {products.length > 0 ? products.map((p) => (
                                                ...
                                                <td className="py-4 px-6">
                                                    <div className="flex flex-col gap-0.5">
                                                        <span className="text-[11px] text-gray-600">
                                                            {p.region || 'Neznámý region'}
                                                        </span>
                                                        <span className="text-[10px] text-gray-400">
                                                            {p.alcohol}% alc | {p.volume}l
                                                        </span>
                                                    </div>
                                                </td>
                                            ...
                                            <tr>
                                                <td colSpan={4} className="py-12 text-center text-gray-400 italic">Zatím žádné produkty</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
